...
## 🖥 いちばん簡単：ローカルWebアプリ（フォームにキーを入れて実行）

「フォームにAPIキーを入れてボタンで実行」したいならこれ。**あなたのPC上で動くWebアプリ**で、
キーはこのPCの中だけに保持され、ブラウザや外部には出ません（公開アーティファクトと違いローカルなので安全に実行可）。

```bash
cd tsuihai-marketer
export X_BEARER_TOKEN='あなたのBearer'   # ← 事前に入れておけばフォームは空でOK
export XAI_API_KEY='あなたのGrokキー'     # 精度用・任意
python3 tmark_web.py                      # → ブラウザで http://127.0.0.1:8787 を開く
```

ブラウザで **プロダクト・自分のhandle・人数・予算** を入れて「実行する」を押すと、
X APIで収集（予算内で自動停止）→ **ユーザー別TOP出し** が画面に出ます。
キーを環境変数に入れたくなければ、画面下の「APIキーをこの画面で入れる」に貼ってもOK
（送信先はこのPCのローカルサーバのみ）。

> これは**手元で動くローカルサーバ**だからキー入力＆実行ができます。
> 一方、claude.ai上の**公開アーティファクトは外部通信不可**なので、そちらはCLIが出力した
> `data/` のJSONLを読み込む方式です（下記CLI手順の6）。
